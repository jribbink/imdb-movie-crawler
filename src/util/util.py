        ...
        if hasattr(video, "info") and type(video.info) is list:
            video.info = VideoInfo(
                **{
                    ...
                    if "description" in video.info
                    else None,
                    "imdb_title": video.info["imdb_title"]
                    if "imdb_title" in video.info
                    else None,
                    "directors": video.info["directors"]
                    if "directors" in video.info
                    else None,
                    "writers": video.info["writers"]
                    if "writers" in video.info
                    else None,
                    ...
                    "film_length": video.info["film_length"]
                    if "film_length" in video.info
                    else None,
                    "parental_rating": video.info["parental_rating"]
                    if "parental_rating" in video.info
                    else None,
                    "release_info": video.info["release_info"]
                    if "release_info" in video.info
                    else None,
                    "image": video.info["image"] if "image" in video.info else None,
                    "imdb_url": video.info["imdb_url"]
                    if "imdb_url" in video.info
                    else None,
                }
            )
            upgrade_count += 1
...
"""
Load people from data file
"""


def load_people(filename="dumppeople.file") -> "list[Video]":
    people = None
    output_file = os.path.join(config["OUTPUT"]["output_location"], filename)
    if os.path.isfile(output_file):
        with open(output_file, "rb") as f:
    ...
    people_file = open("config/pa_directors.json")
    people = []
    for person in json.load(people_file):
        people.append(Person(**person))
    return people


"""
Save people to data file
"""


def dump_people(people: "list[Video]", filename="dumppeople.file"):
    location = os.path.join(config["OUTPUT"]["output_location"], filename)
    backup = os.path.dirname(location) + "." + os.path.basename(location) + ".bck"
    bck_idx = 1
