...
class VideoCrawlerThreadpool:
    ...
    def run(self):
        for i in range(0, self.num_threads):
            thread = VideoCrawlerThread(self, self.crawler_options, i)
            thread.start()
...
class VideoCrawlerThread(threading.Thread):
    def __init__(
        self, parent: VideoCrawlerThreadpool, crawler_options: dict, index: int
    ):
