...
class QueryInfo:
    ...
    def query(self):
        if self.__query_index >= len(self.__queries):
            return None
        print(self.__query_index)
        return self.__queries[self.__query_index].format(
            title=self.title, year=self.year, type=self.type
        )
...
class VideoInfo:
    def __init__(self, *args, **kwargs):
        ...
        self.directors: list[str] = (
            kwargs["directors"] if "directors" in kwargs else None
        )
        ...
        self.parental_rating = (
            kwargs["parental_rating"] if "parental_rating" in kwargs else None
        )
