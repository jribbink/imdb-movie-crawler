...
from util.pandas_helper import get_variation_df, videos_to_data_frame
...
def csv_dump():
    ...
    indices = request_range(
        "Please enter the range of videos you would like to export (i.e. 0-2) (leave blank for all videos): ",
        default=range(0, len(videos) - 1),
    )

    csv_name = request_input(
        "What would you like to name your CSV (default out.csv)? ", default="out.csv"
    )
    ...
    df_var = get_variation_df(videos)
    df_var.to_csv("output/out-var.csv")
